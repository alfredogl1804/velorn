 ...
 *   external - HTTP ready but the process was started outside Monstruo Studio
 ...
 * when we've reclaimed a ComfyUI from a previous Monstruo Studio session.
 */
...
class ComfyLauncher extends EventEmitter {
  ...
  async init() {
    ...
    // back to plain external detection. This handles the "Monstruo Studio
  ...
  async _detectExternal() {
    ...
    // This covers the "Monstruo Studio crashed, ComfyUI kept running, no state
    ...
    if (pid) {
      ...
      this._setState('running', {
        statusMessage: `Connected to running ComfyUI (pid ${pid}). Monstruo Studio didn't start it, but can stop or restart it.`,
      })
      return
    }
  ...
  /**
   * Called once during init(). If a previous Monstruo Studio session wrote a
  ...
  async start() {
    ...
    // Monstruo Studio session crashed and left ComfyUI alive), adopting it is
    ...
    // Persist pid/port so a future Monstruo Studio boot can reclaim this child
  ...
  async _spawnLauncher(launcherScript, config) {
    ...
    if (portable) {
      ...
      // --enable-cors-header "*" is required for Monstruo Studio's renderer to
      ...
      return spawn(portable.pythonExe, mergedArgs, {
        ...
        // Monstruo Studio is running; on detach we release them explicitly.
        detached: true,
  ...
  async stop() {
    ...
    if (this._state === 'external') {
      return { success: false, error: 'ComfyUI was started outside of Monstruo Studio. Stop it from the window where you started it.' }
    }
    if (!this._child && !this._pid) {
      return { success: false, error: 'No ComfyUI process is currently owned by Monstruo Studio.' }
    }
    ...
    // Reclaimed case: we only have the PID (previous Monstruo Studio session
  ...
  /**
   * Release the child process so it keeps running after Monstruo Studio quits.
   *
  ...
  async detach() {
    ...
    const pid = this._pid
    this._appendLog('system', 'Detach requested — leaving ComfyUI running after Monstruo Studio quits.')
    ...
    // Flip ownership to "external" so if Monstruo Studio is relaunched while
    // ComfyUI is still up, the normal detectExternal path adopts it.
    this._ownership = 'external'
    this._child = null
    this._setState('external', {
      statusMessage: pid
        ? `ComfyUI left running (pid ${pid}). It will continue after Monstruo Studio quits.`
        : 'ComfyUI left running. It will continue after Monstruo Studio quits.',
    })
