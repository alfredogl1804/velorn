...
export function buildPremiereXml({
  ...
  const projectLabel = sanitizeName(projectName, 'Monstruo Studio Project')
  ...
  return [
    ...
    `      </audio>`,
    `    </media>`,
    `    <description>Exported from Monstruo Studio project: ${escapeXml(projectLabel)}</description>`,
    `  </sequence>`,
    `</xmeml>`,
    '',
  ].join('\n')
