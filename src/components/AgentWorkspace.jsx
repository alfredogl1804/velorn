...
const BASE_AGENT_PROMPT = `You are Monstruo Studio Agent, an AI assistant built into Monstruo Studio.

You help the user inspect and operate the open Monstruo Studio project using the Monstruo Studio tools available to you.
...
function formatToolResultForChat(toolName, result, summary = '') {
  ...
  return [
    `Monstruo Studio tool result for ${toolName}:`,
...
function cleanAssistantText(text) {
  ...
  cleaned = cleaned.replace(/<channel>\s*(?:final|answer|assistant)\s*/gi, '')
  cleaned = cleaned.replace(/<\/?channel[^>]*>/gi, '')
  cleaned = cleaned.replace(/Monstruo Studio tool result for [\s\S]*$/gi, '')
...
function shouldUseToolSummary(response) {
  ...
  if (/```|<channel>|<think>|Monstruo Studio tool result|Technical result JSON/i.test(text)) return true
...
function AgentWorkspace() {
  ...
  const executeToolCalls = async (toolCalls, baseMessages) => {
    ...
    for (const call of toolCalls) {
      ...
      try {
        ...
        resultMessages.push(makeMessage('tool', `Monstruo Studio tool error for ${toolName}:\n${error.message || String(error)}`, {
          toolName,
          status: 'error',
  ...
  const runConversation = async (startingMessages) => {
    ...
    const summaryPrompt = `Do not call any tools in this response. Do not reveal chain-of-thought or mention JSON/tool blocks. Summarize the Monstruo Studio tool results in plain English in 1-2 sentences. Put the answer first. If the user asked a simple count, answer the count first. If a tool was previewOnly, say that nothing was applied yet and what the user should approve next.`
    ...
    <div className="flex h-full min-h-0 w-full flex-1 bg-sf-dark-950 text-sf-text-primary">
        ...
        <div className="border-b border-sf-dark-700 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-sf-text-muted">Local AI control for the open Monstruo Studio project.</p>
            </div>
          </div>
        </div>
      ...
      <main className="relative flex min-w-0 flex-1 flex-col bg-[radial-gradient(circle_at_top,rgba(117,92,255,0.12),transparent_40%),#05070d]">
        <div className="flex items-center justify-between border-b border-sf-dark-700 bg-black/25 px-5 py-3">
          <div>
            <h2 className="text-sm font-semibold">Monstruo Studio Agent</h2>
            <p className="text-xs text-sf-text-muted">
              Ask a local model to inspect, plan, preview, and operate the current project through Monstruo Studio tools.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
            ...
            <div className="w-full max-w-2xl rounded-xl border border-amber-300/25 bg-sf-dark-900 p-6 shadow-2xl">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-sf-text-muted">This gives a local AI model tool access to the open Monstruo Studio project.</p>
                </div>
              </div>
              <div className="space-y-3 text-sm leading-relaxed text-sf-text-secondary">
                <p>
                  ...
                  label clips, add markers, change keyframes, queue generations, and start exports through Monstruo Studio tools.
                </p>
                <p>
                  It does not expose generic shell, operating-system, or arbitrary file tools from Monstruo Studio. Still, for important
          ...
          {messages.length === 0 && !currentResponse ? (
            <div className="mx-auto mt-14 max-w-2xl text-center">
              ...
              <p className="text-sm leading-relaxed text-sf-text-muted">
                Start with a question, a review pass, or a preview-only edit. The model will call Monstruo Studio tools when it needs project context or editor control.
              </p>
            </div>
          ) : (
